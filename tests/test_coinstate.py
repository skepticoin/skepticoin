...
from skepticoin.coinstate import PKBalance, pkb_apply_transaction, uto_apply_transaction
from skepticoin.consensus import construct_reference_to_thin_air
from skepticoin.datatypes import Input, Output, OutputReference, Transaction
...
def test_uto_apply_transaction_on_coinbase():
    ...
    result = uto_apply_transaction(
        unspent_transaction_outs, transaction, is_coinbase=True
    )
...
def test_pkb_apply_transaction_on_coinbase():
    ...
    result = pkb_apply_transaction(
        unspent_transaction_outs, public_key_balances, transaction, is_coinbase=True
    )
...
def test_pkb_apply_transaction_on_non_coinbase_transaction():
    ...
    result = pkb_apply_transaction(
        unspent_transaction_outs, public_key_balances, transaction, is_coinbase=False
    )

    assert (
        result[public_key_0].value == 0
    )  # not referenced in the transaction under consideration
