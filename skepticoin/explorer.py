    ...
    with public_key_balances.mutate() as mutable_public_key_balances:
        ...
        for i, output in enumerate(transaction.outputs):
            ...
            mutable_public_key_balances[output.public_key] = PKBalance2(
                ...
                + output.value,
                all_output_references=(
                    mutable_public_key_balances[output.public_key].all_output_references
                    + [output_reference]
                ),
                unspent_output_references=(
                    mutable_public_key_balances[
                        output.public_key
                    ].unspent_output_references
                    + [output_reference]
                ),
...
def build_pkb2(coinstate):
    ...
    for height in range(coinstate.head().height + 1):
        ...
        public_key_balances_2 = build_pkb2_block(
            coinstate, block, public_key_balances_2
        )
...
"""
            ...
            for transaction in block.transactions:
                ...
                with open(
                    Path("explorer") / (human(transaction.hash()) + ".md"), "w"
                ) as transaction_f:
                    transaction_f.write(
