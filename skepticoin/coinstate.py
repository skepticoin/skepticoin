...
def uto_apply_block(unspent_transaction_outs, block):
    ...
    for transaction in block.transactions[1:]:
        unspent_transaction_outs = uto_apply_transaction(
            unspent_transaction_outs, transaction, is_coinbase=False
        )
    return unspent_transaction_outs


def pkb_apply_transaction(
    unspent_transaction_outs, public_key_balances, transaction, is_coinbase
):
    with public_key_balances.mutate() as mutable_public_key_balances:
        # for coinbase we must skip the input-removal because the input references "thin air" rather than an output.
        if not is_coinbase:
            for input in transaction.inputs:
                previously_unspent_output = unspent_transaction_outs[
                    input.output_reference
                ]

                public_key = previously_unspent_output.public_key
                mutable_public_key_balances[public_key] = PKBalance(
                    mutable_public_key_balances[public_key].value
                    - previously_unspent_output.value,
                    [
                        to
                        for to in mutable_public_key_balances[
                            public_key
                        ].output_references
        ...
        for i, output in enumerate(transaction.outputs):
            ...
            mutable_public_key_balances[output.public_key] = PKBalance(
                ...
                mutable_public_key_balances[output.public_key].output_references
                + [output_reference],
            )
...
def pkb_apply_block(unspent_transaction_outs, public_key_balances, block):
    ...
    for transaction in block.transactions[1:]:
        public_key_balances = pkb_apply_transaction(
            unspent_transaction_outs, public_key_balances, transaction, False
        )
...
class CoinState:
    ...
    def add_block_no_validation(self, block: Block) -> None:
        if block.previous_block_hash == b"\00" * 32:
            ...
            unspent_transaction_outs = self.unspent_transaction_outs_by_hash[
                block.previous_block_hash
            ]
            public_key_balances = self.public_key_balances_by_hash[
                block.previous_block_hash
            ]

        public_key_balances = pkb_apply_block(
        ...
        if block.previous_block_hash == b"\00" * 32:
            block_by_height_by_hash = immutables.Map(
                {block.hash(): immutables.Map({0: block})}
            )
        else:
            ...
            block_by_height_by_hash = self.block_by_height_by_hash.set(
                block.hash(), block_by_height
            )
        ...
        if (
            self.current_chain_hash is None
            or self.current_chain_hash == block.previous_block_hash
        ):
        ...
        # broken based on first-come-first serve. I'm sure someone wrote a paper on how this is optimal.
            ...
            block.get_total_work()
            > self.block_by_hash[self.current_chain_hash].get_total_work()
        ):
            current_chain_hash = block.hash()
        else:
            current_chain_hash = (
    ...
    def forks(self):
        def _find_lca_with_main(block):
            ...
            while (
                block.height not in self.by_height_at_head()
                or self.by_height_at_head()[block.height].hash() != block.hash()
            ):
